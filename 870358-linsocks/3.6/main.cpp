...
Добавить к программе из п.3.5 получение адреса, автоматически назначенного локальному сокету, 
с помощью функции getsockname(), и вывод его в строковом формате
*/
...
int main(int argc, char* argv[])
{
	...
	do { // только для того, чтобы удобно было при ошибке выйти по break сразу на освобождение ресурсов
		...
		if(getsockname(sock, (struct sockaddr *)&addr_in, &namelen) != 0) {
			cerr << "getsockname failed" << endl;
			break;
		}
		if(!inet_ntop(AF_INET, &addr_in.sin_addr.s_addr, str, sizeof(str))) {
			cerr << "inetntop failed" << endl;
			break;
		}
		cout << "\nlocal address = " << str << endl;
