...
DWORD WINAPI TcpSrv::WorkerThread(LPVOID lpParam)
{
	...
	while (TRUE) {
		...
		case ClientIoRead: {
			...
			// в начале оставляем 4 байта для длины сообщения
			...
			case reqOsVer:
				...
				if (GetVersionEx((LPOSVERSIONINFO)&pReply->vf.osVer)) {
					pReply->vf.osVer.szCSDVersion[127] = TEXT('\0'); // на всякий случай
			...
			case reqObjectOwn:
				if (pRequest->pathSize) {
					...
					if (pObjectOwner && cbReplyBody) {
						...
						wcscpy_s(pReply->vf.objectOwner, cbReplyBody, pObjectOwner);
						cbReplyBody *= sizeof(TCHAR); // перевод в байты (важно для Юникода)
					}
					else {
						resultCode = (int)GetLastError();
						cerr << "GlobalMemoryStatusEx failed: " << resultCode << endl;
