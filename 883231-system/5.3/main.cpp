/*
5.3. Добавьте отключение буферизации в программу для предыдущего 
задания и проведите те же измерения с отключенной буферизацией.
...
bool copy(const char* source, const char* destin, size_t buf_size, size_t block_size = 1) {
	...
	do {
		...
		// отключаем буферизацию для исходного файла
		if(setvbuf(read_file, NULL, _IONBF, 0)) {
			fprintf(stderr, "failed to switch off buffer for \"%s\"\n", source);
			break;
		}
		...
		// отключаем буферизацию для конечного файла
		if(setvbuf(read_file, NULL, _IONBF, 0)) {
			fprintf(stderr, "failed to switch off buffer for \"%s\"\n", destin);
			break;
		}
