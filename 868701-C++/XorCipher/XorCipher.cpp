...
void makeParams(generator_t& gen, const string& fileName) {
	if (fileExists(fileName)) {
		cout << "Файл \"" << fileName << "\" уже существует, продолжить (y/n)? ";
		char answer;
		cin >> answer;
		if (tolower(answer) == 'n') {
			return;
...
int main()
{
	...
	while (menu != 0) {
		...
		switch (menu) {
		case 1:
			dataFileName = inputFileName("Введите имя файла данных: ");
			break;
		case 2:
			keyFileName = inputFileName("Введите имя файла параметров: ", "key");
			break;
		case 3:
