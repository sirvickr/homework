...
// Формирование передаваемого вектора, пока с нулевыми проверочными битами
void initCodeSeq(const bits_t& b, bits_t& bt, size_t N) {
	for (size_t k = 0, j = 0; j < N; j++) {
		size_t v = j + 1;
		// проверка, не является ли номер степенью двойки
		if ((v && !(v & (v - 1)))) {
			bt[j] = 0; // является, обнуляем место для проверочного бита
		}
		else {
